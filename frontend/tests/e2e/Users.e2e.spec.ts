...
test.describe("Usuários", () => {
  test("navega para Usuários e lista itens do backend", async ({ page }) => {
    ...
    await page.getByRole("link", { name: "Usuários" }).click(); // Título da seção
    await expect(
      page.getByRole("heading", { name: /Usuários/i })
    ).toBeVisible(); // Emails semeados (seed do backend)
  ...
  test("cria usuário e aparece na lista", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /Criar/i }).click(); // Aguarda recarga da lista
  ...
  test("atualiza um usuário existente", async ({ page }) => {
    ...
    await page.getByLabel("Nome:").fill("Usuário Atualizado");
